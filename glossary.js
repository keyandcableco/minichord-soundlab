...
(function () {
  ...
  const TERMS = {
    ...
    // tuning & temperament
    temperament: { short: "How the twelve notes of the octave are spaced. Equal temperament makes every step identical; the older ones favour some keys over others to get purer chords.", long: "Pure intervals are simple frequency ratios \u2014 a fifth is 3:2, a major third 5:4. You cannot have all of them in tune at once on twelve fixed notes, so every tuning is a choice about which to favour. Anchored on C here, so the unequal ones sound different in different keys.", see: ["cents", "meantone", "just intonation", "equal temperament"] },
    "equal temperament": { short: "Twelve identical steps per octave. Every key sounds the same, and no interval except the octave is quite in tune.", long: "A major third comes out 13.7 cents sharp of pure, which is the faint beating in a piano chord. The compromise that let music move freely between keys.", see: ["temperament", "cents", "meantone"] },
    meantone: { short: "A tuning that makes major thirds pure by narrowing the fifths slightly. What most Renaissance and early Baroque music was written for.", long: "Quarter-comma meantone gives an exactly pure 5:4 third, paid for with fifths about 5 cents flat. Keys far from the anchor get progressively stranger, which is why that music tends to stay near home.", see: ["temperament", "cents", "just intonation"] },
    "just intonation": { short: "Tuning built from simple whole-number frequency ratios, so intervals in the home key are exactly in tune.", long: "Beautiful where it is aimed and noticeably out elsewhere, because the ratios that suit one key do not suit another.", see: ["temperament", "harmonic", "meantone"] },
    pythagorean: { short: "A tuning built from pure 3:2 fifths stacked all the way round, giving bright, wide major thirds.", long: "Thirds land 21.5 cents sharp, which suits medieval music written around fifths and fourths rather than triads.", see: ["temperament", "cents", "fifth"] },
    cents: { short: "A unit for measuring pitch difference: a hundred cents is one equal-tempered semitone, twelve hundred is an octave.", long: "About five cents is the smallest difference most people notice on a sustained note; the 13.7-cent error in an equal-tempered third is heard as beating rather than as wrong pitch.", see: ["temperament", "frequency", "octave"] },
